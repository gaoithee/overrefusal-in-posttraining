...
def load_full_dataset(hf_dataset, layers, position, token, extra_cols=None):
    """
    Carica l'intero dataset HF (split='train').
    ...
    print(f"[.] Carico dataset '{hf_dataset}' (split=train) ...", end=" ", flush=True)
    try:
...
def main():
    ...
    df_full = load_full_dataset(
        args.hf_dataset, args.layers, args.position, token, extra_cols=extra_cols
    )
    if df_full is None:
        return
    ...
    if args.split_by == "none":
        ...
        subsets = [(f"source={s}", lambda proj, s=s: proj["source"] == s)
                   for s in sources]
        # Aggiungi anche la figura completa come riferimento
    ...
    for label, mask_fn in subsets:
        ...
        excl_label = ",".join(sorted(args.exclude_from_directions))                      if args.exclude_from_directions else ""
        plot_grid(
            results,
